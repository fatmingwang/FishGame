...
{
	POINT	GetPoint(const WCHAR* e_str){return GetPoint(UT::WcharToChar(e_str).c_str());}
	...
	float	GetFloat(const WCHAR* e_str){return GetFloat(UT::WcharToChar(e_str).c_str());}
	...
	int		GetInt(const WCHAR* e_str){return GetInt(UT::WcharToChar(e_str).c_str());}
	int		GetInt(std::wstring e_str){ return GetInt(e_str.c_str()); }
	int		GetInt(std::string e_str){ return GetInt(e_str.c_str()); }

	uint64	GetUint64(const WCHAR* e_str){return GetUint64(UT::WcharToChar(e_str).c_str());}
	...
	int64	GetInt64(const WCHAR* e_str){return GetInt64(UT::WcharToChar(e_str).c_str());}
	...
	RECT	GetRectangle(const WCHAR*e_str){return GetRectangle(UT::WcharToChar(e_str).c_str());}
	...
	Vector4	GetVector4(const WCHAR*e_strValue){return GetVector4(UT::WcharToChar(e_strValue).c_str());}	

	Vector3	GetVector3(const WCHAR*e_strValue){return GetVector3(UT::WcharToChar(e_strValue).c_str());}

	Vector2	GetVector2(const WCHAR*e_strValue){return GetVector2(UT::WcharToChar(e_strValue).c_str());}	
	...
	cMatrix44	GetMatrix(const WCHAR*e_str,bool e_bTranspose){return GetMatrix(UT::WcharToChar(e_str).c_str(),e_bTranspose);}
	...
	void	GetUV(const WCHAR*e_pData,float*e_pUVBuffer){return GetUV(UT::WcharToChar(e_pData).c_str(),e_pUVBuffer);}
...
#endif
		std::wstring	l_str;
		WCHAR	l_temp[TEMP_SIZE];
		memcpy(l_temp,e_strValue,sizeof(WCHAR)*l_iLength);
		l_temp[l_iLength] = L'\0';
		l_str = l_temp;
		return l_str;
	}
	...
	{
		char	l_temp[TEMP_SIZE];
		sprintf(l_temp,"%s\0",UT::WcharToChar(e_strValue).c_str());
		std::string	l_str2 = l_temp;
		return l_str2;	
	}
