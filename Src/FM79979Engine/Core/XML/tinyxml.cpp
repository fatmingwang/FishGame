...
void TiXmlElement::SetAttribute( const WCHAR* e_strName, std::wstring e_strValue )
{
	SetAttribute( e_strName, e_strValue.c_str() );
}
void TiXmlElement::SetAttribute( std::wstring e_strName, const WCHAR* e_strValue )
{
...
bool TiXmlDocument::LoadFile( char*e_strData,int e_iLength)
{
	...
	WCHAR* buf = new WCHAR[ l_iLength+1 ];//assume all file is unicode
...
#endif
	{
		...
		WCHAR* pNewbuf = new WCHAR[ l_iLength+1 ];
	...
	if( buf[1] == 0 )
	{
		...
		{
			WCHAR* l_pNewBuffer = new WCHAR[ l_iLength/2+1 ];//assume all file is unicode
			int	l_iIndex = 0;
...
bool TiXmlDocument::LoadFile( NvFile* file)
{
	...
	WCHAR* buf = new WCHAR[ length ];//assume all file is unicode
