...
namespace FATMING_CORE
{
	...
	void    cEventVariableDataRender::InternalRender()
	{
		...
		if(this->m_pEventVariable)
		{
			...
			{
				g_pCurrentEventManager->m_pEventFont->RenderFont(this->m_vPos.x-l_vRenderSizeOffset.x,this->m_vPos.y,l_str.c_str());
			}
		}
		else
		{
			g_pCurrentEventManager->m_pEventFont->RenderFont(this->m_vPos.x-l_vRenderSizeOffset.x,this->m_vPos.y,L"m_pEventVariable not exist");
		}
	}
