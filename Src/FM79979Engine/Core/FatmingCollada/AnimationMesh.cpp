...
void cAnimationMesh::UpdateNodes(float e_fTimeValue)
{
    ...
    for(int i=0;i<l_iNum;++i)
    {
		m_SkinningBoneVector[i]->EvaluateLocalXForm(e_fTimeValue);
...
void	cAnimationMesh::Render(WCHAR*e_strShaderName)
{
	...
	if( !l_bTextureExist )
	{
		...
		{
			cSimpleGLTexture*l_pSimpleGLTexture = dynamic_cast<cSimpleGLTexture*>(cGameApp::m_spImageParser->GetObject(0));
