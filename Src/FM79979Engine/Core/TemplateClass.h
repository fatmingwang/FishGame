...
public:
	...
	inline	bool	RemoveObjectWithoutDelete(const WCHAR*e_strName){ return RemoveObjectWithoutDelete((WCHAR*)e_strName); }
	inline	bool	RemoveObjectWithoutDelete(WCHAR*e_strName){int	l_iIndex = GetObjectIndexByName(e_strName);if( l_iIndex != -1 )	return RemoveObjectWithoutDelete(l_iIndex);return false;}
