...
namespace FATMING_CORE
{
	...
	{
		if(e_pObject->Type() == cSimpleGLTexture::TypeID)
		{
			cSimpleGLTexture*l_pSimpleGLTexture = dynamic_cast<cSimpleGLTexture*>(e_pObject);
	...
	void	cImageParser::HandleElementData(TiXmlElement *e_pTiXmlElement)
	{
		...
		COMPARE_NAME("PuzzleImage")
		{
			...
			const WCHAR*l_strImageName = this->m_pCurrentTiXmlElement->Attribute(L"ImageName");
			if( l_strImageName )
			{
	...
	//</PuzzleImage>
			...
			{
				cNumeralImage*l_pNumerialImage = dynamic_cast<cNumeralImage*>(l_pNamedTypedObject);
