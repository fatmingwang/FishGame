...
namespace FATMING_CORE
{
	...
	cPuzzleImageUnit*		cPuzzleImageUnit::GetMe(TiXmlElement*e_pElement)
	{
		...
		PARSE_ELEMENT_START(e_pElement)
			COMPARE_NAME("Name")
			{
				if( l_pPuzzleImageUnit )
					l_pPuzzleImageUnit->SetName(l_strValue);
			}
			else
			COMPARE_NAME("PI")
			{
			...
			{
				if( l_pPuzzleImageUnit )
					l_pPuzzleImageUnit->SetPos(VALUE_TO_VECTOR3);
			}
			else
			COMPARE_NAME("Color")
			{
				if( l_pPuzzleImageUnit )
					l_pPuzzleImageUnit->SetColor(VALUE_TO_VECTOR4);
			}
	...
	cNumeralImage*	cPuzzleImage::GetNumerialImageByName(const WCHAR*e_str0ImaneName,const WCHAR*e_str9ImaneName)
	{
		...
		{
			cNumeralImage*l_pNumerialImage = new cNumeralImage(l_pPuzzleImageUnit0,l_pPuzzleImageUnit9);
			return l_pNumerialImage;
		}
		return 0;	
	}

	cNumeralImage*	cPuzzleImage::GetNumerialImageByName(const WCHAR*e_strNumerImageName)
	{
		cPuzzleImageUnit*l_pPuzzleImageUnit = GetObject(e_strNumerImageName);
		if( l_pPuzzleImageUnit )
		{
			cNumeralImage*l_pNumerialImage = new cNumeralImage(l_pPuzzleImageUnit,l_pPuzzleImageUnit->GetUV());
			return l_pNumerialImage;
		}
		return 0;
	}
	
	std::string       cPuzzleImage::GetFileName(const WCHAR*e_strObjectName)
	{
