...
namespace FATMING_CORE
{
    ...
    //=====================
    class	cFMAnimationRule:public cRenderObjectBehavior
    {
    protected:
    ...
    public:
	    cFMAnimationRule();
	    cFMAnimationRule(cFMAnimationRule*e_pFatmingAnimationRule);
	    ~cFMAnimationRule();
    ...
    //======================================
    //basicly the TYPE is a inherit class base on cFMAnimationRule
    //this is a list to update and render all animation rule objects
    //it also have timing base function for the objects.
    //======================================
    template<class	TYPE>
    class	cFatmingGroupBehaviorList:public cNamedTypedObjectVector<TYPE>,public cFMAnimationRule
    {
    ...
    public:
		...
		{
			cFMAnimationRule::m_fStartTime = e_fStartTime;
			m_fTotalPlayTime = e_fEndTime; 
		}

	    virtual	void				SetAnimationLoop(bool e_bLoop);
	    virtual	void				Stop(){ m_fPastTime = m_fTotalPlayTime+1.f; m_iCurrentPlayCount = m_iPlayCount; m_fCurrentProgress = 1.f;SetAnimationDone(true); }
	...
	:cFMAnimationRule(e_pFatmingGroupBehaviorList)
	//:cNamedTypedObjectVector<TYPE>(e_pFatmingGroupBehaviorList),cFMAnimationRule(e_pFatmingGroupBehaviorList),the reason why not calling cNamedTypedObjectVector<TYPE>(e_pFatmingGroupBehaviorList) is because AddObjectNotification
    {
    ...
    void	cFatmingGroupBehaviorList<TYPE>::InternalInit()
    {
		...
		{
		    ((cFMAnimationRule*)this->m_ObjectList[i])->Init();
		}
		if( m_pStayAtLastFrameOrLoopChildrenVector->Count() == 0 )
		{
			...
			{
				((cFMAnimationRule*)this->m_ObjectList[i])->Init();
				if(((cFMAnimationRule*)this->m_ObjectList[i])->IsStayAtLastFrame() || ((cFMAnimationRule*)this->m_ObjectList[i])->IsAnimationLoop())
				{
	...
	{
		cFMAnimationRule::Update(e_fElpaseTime);
		if( m_bAnimationDone )
		{
			int	l_iCount = m_pStayAtLastFrameOrLoopChildrenVector->Count();
			for( int i=0;i<l_iCount;++i )
			    ((cFMAnimationRule*)(*m_pStayAtLastFrameOrLoopChildrenVector)[i])->Update(e_fElpaseTime);
		}
	}
    //======================================
    //
    ...
    void	cFatmingGroupBehaviorList<TYPE>::InternalUpdate(float e_fElpaseTime)
    {
	    if( m_iPlayCount != m_iCurrentPlayCount )
	    {
		    ...
		    for( UINT i=0;i<l_iCount;++i )
			    ((cFMAnimationRule*)this->m_ObjectList[i])->Update(e_fElpaseTime);
	...
	template<class	TYPE>
	void	cFatmingGroupBehaviorList<TYPE>::SetPlayCount(int e_iPlayCount)
	{
	...
	{
		cFMAnimationRule::Render();
		if( m_bAnimationDone )
		{
			int	l_iCount = m_pStayAtLastFrameOrLoopChildrenVector->Count();
			for( int i=0;i<l_iCount;++i )
			    ((cFMAnimationRule*)(*m_pStayAtLastFrameOrLoopChildrenVector)[i])->Render();
		}
	}
    //======================================
    //
    ...
    void	cFatmingGroupBehaviorList<TYPE>::InternalRender()
    {
	    ...
	    for( UINT i=0;i<this->m_ObjectList.size();++i )
	    {
		    ...
		    if( l_pTYPE->IsAnimationDone() )
			    continue;
		    ((cFMAnimationRule*)l_pTYPE)->Render();
	    }
    }
    ...
    template<class	TYPE>
    void	cFatmingGroupBehaviorList<TYPE>::SetAnimationLoop(bool e_bLoop)
	{
		cFMAnimationRule::SetAnimationLoop(e_bLoop);
		if(e_bLoop)m_iPlayCount = -1;else m_iPlayCount = 1;
	}
    //======================================
    //
    //======================================
    ...
    void	cFatmingGroupBehaviorList<TYPE>::RearrangeTimeByPercent(float e_fPercenttage)
	{
	    ...
	    {
			cFMAnimationRule::m_fStartTime *= e_fPercenttage;
		    int	l_iNum = this->Count();
		    for( int i=0;i<l_iNum;++i )
		    {
			    TYPE*	l_pType = (*this)[i];
