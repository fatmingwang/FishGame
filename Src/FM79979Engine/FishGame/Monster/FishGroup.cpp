...
{
	Vector3	l_vStartPos = Vector3(0.f,-cGameApp::m_svGameResolution.y,0.f);
	Vector3	l_vEndPos = Vector3(0.f,cGameApp::m_svGameResolution.y,0.f);
...
void	cFishGroup::f_PrizeII_Go( cCurveWithTime e_TargetPath )
{
	...
	for( size_t	i=0; i<(size_t)m_FishGroup.Count(); ++i ) //how many index in mpdi
	{
		...
		cFishGroup::cFishGroupPointData* l_pFishGroupPointData1 = this->m_FishGroup.GetObject(i);
