...
int	cBluffingDiceAI::MatchDicePointCount(int e_iPlayerIndex,int e_iCurrentDicePoint,bool e_bSpecialOne,int e_iVisibleCount)
{
	...
	if( e_iPlayerIndex != -1 && (int)m_DiceTableVector.size() > e_iPlayerIndex )
	{
		...
		for( size_t j=0;j<l_pDiceTable->m_MyDiceContent.size();++j )
		{
			int	l_iDicePoint = l_pDiceTable->m_MyDiceContent[j];
...
bool	cBluffingDiceAI::NewDiceDataByMoseSameDices(bool e_bSpecialOneRule,int e_iDiceAIIndex,int e_iCurrentDicePoint,int e_iCurrentDiceCount,int*e_piNewDiceCount,int*e_piNewDicePoint)
{
	std::vector<int>	l_AllDiceCount;
	l_AllDiceCount.resize(this->m_pDiceRule->iMax);
	for( size_t i=0;i<l_AllDiceCount.size();++i )
		l_AllDiceCount[i] = 0;
	std::vector<int>l_MyDiceContent = m_DiceTableVector[e_iDiceAIIndex]->m_MyDiceContent;
	...
	for( size_t i=0;i<l_MyDiceContent.size();++i )
	{
		...
		int l_iNumSame = l_AllDiceCount[l_iDicePoint];
...
{
	if( e_iCurrentDiceCount >= 3 && m_bEnableToCheat )
	{
			...
			{//force to catch.
				...
				if( l_iNumPlayerHas < e_iCurrentDiceCount )
				{
					...
					{//all AI reset data
						...
						m_DiceTableVector[i]->SetDiceContent(l_Result);
	...
	if( l_iRestGap > 0 )
	{
		...
		{
			if(!NewDiceDataByMoseSameDices(e_bSpecialOneRule,e_iDiceAIIndex,e_iCurrentDicePoint,e_iCurrentDiceCount,&l_iNewCount,&l_iNewPoint))
			{
...
bool	cBluffingDiceAI::IsCatchWin(int e_iCount,int e_iPoint,bool e_bSpecialDiceRule)
{
	...
	for( size_t i=0;i<l_iDiceTableSize;++i )
	{
		...
		for(size_t j=0;j<l_pDiceTable->m_MyDiceContent.size();++j)
		{
			int	l_iPoint = l_pDiceTable->m_MyDiceContent[j];
			if( l_iPoint == g_iSpecialDiceRulePoint && e_bSpecialDiceRule )
			{//special rule
				++l_iNumPoint;
			}
			else
			if( l_pDiceTable->m_MyDiceContent[j] == e_iPoint )
				++l_iNumPoint;
		}
	}
...
cAIDiceTableCallResult::cAIDiceTableCallResult(cBluffingDiceAI*e_pBluffingDiceAI,TiXmlElement*e_pTiXmlElement,int e_iAIIndex)
{
	...
	while( e_pTiXmlElement )
	{
		...
		{
			m_pMyDiceTable = new cDiceTable(e_pTiXmlElement,e_pBluffingDiceAI->m_pDiceRule);
		}
...
void	cAIDiceTableCallResult::AIUpdate(int e_iCurrentDiceCount,int e_iCurrentDicePoint,bool e_bSpecialRule)
{
