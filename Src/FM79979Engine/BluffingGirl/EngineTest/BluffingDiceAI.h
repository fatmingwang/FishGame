...
class	cBluffingDiceAI
{
	...
	bool						NewDiceDataByMoseSameDices(bool e_bSpecialOneRule,int e_iDiceAIIndex,int e_iCurrentDicePoint,int e_iCurrentDiceCount,int*e_piNewDiceCount,int*e_piNewDicePoint);
public:
...
public:
	...
	void				AIUpdate(int e_iCurrentDiceCount,int e_iCurrentDicePoint,bool e_bSpecialRule);
	void				Render();
	void				Init();
