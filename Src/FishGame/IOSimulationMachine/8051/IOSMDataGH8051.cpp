...
namespace IOSM
{
	...
	void	cIOSMIOData::Update(float e_fElpaseTime)
	{
		if( m_pucKeyData  )
		{
			...
			if ( *m_pucKeyData != 38 && *m_pucKeyData != 40 && *m_pucKeyData != 37 && *m_pucKeyData != 39 && *m_pucKeyData != 8 )
	...
	void	cIOSMIODataManager::ProcessIOPackData()
	{
		...
		PARSE_NAME_VALUE_START
			...
			COMPARE_NAME("KeyData")
			{
				...
				*l_pucKeyData = (unsigned char)l_strValue[0];
...
//#endif
	...
	void	cIOSMIODataManager::ProcessIOData()
	{
		...
		PARSE_NAME_VALUE_START
			...
			{
				l_pIOSMIOData->SetWorkingSingnal(_wtoi(l_strValue)?true:false);
			}
			else
			COMPARE_NAME("KeyData")
			{
...
//#endif
	...
	void	cIOSMIODataManager::HandleElementData(TiXmlElement*e_pTiXmlElement)
	{
		...
		COMPARE_VALUE("IODataInfoList")
		{
			...
			const WCHAR*l_strSafeRefundCoinCount = e_pTiXmlElement->Attribute(L"SafeRefundCoinCount");
			if( l_strSafeRefundCoinCount )
			{
				m_iSafeRefundCoinCount = GetInt(l_strSafeRefundCoinCount);
			}
		}
		else
